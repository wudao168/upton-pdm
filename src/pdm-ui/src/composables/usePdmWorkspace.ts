...
export function usePdmWorkspace() {
  ...
  const drawingReviewReviewers = ref<ApprovalTransferCandidate[]>([])
  ...
  async function exportWearPartBomFile(mode: BomExportMode) {
    const { blob, fileName } = await exportWearPartBom(project.value.id, mode, accessToken)
    const url = URL.createObjectURL(blob)
    const anchor = document.createElement('a')
    anchor.href = url
    anchor.download = fileName
  ...
  async function decideDrawingReviewSupervisor(packageId: string, decision: DrawingReviewDecision, comment: string) {
    operationPending.value = true
    operationError.value = ''
    try {
      replaceDrawingReview(await decideDrawingReviewSupervisorRequest(packageId, decision, comment, accessToken))
    } catch (error) {
      operationError.value = messageFrom(error)
      throw error
    } finally { operationPending.value = false }
  ...
  async function resubmitDrawingReviewItem(packageId: string, itemId: string) {
    operationPending.value = true
    operationError.value = ''
    try {
      replaceDrawingReview(await resubmitDrawingReviewItemRequest(packageId, itemId, accessToken))
      drawingReviewCandidates.value = await listDrawingReviewCandidates(project.value.id, accessToken)
    } catch (error) {
      operationError.value = messageFrom(error)
      throw error
  ...
  async function listU9SyncBlockers() {
    return listU9SyncBlockersRequest(project.value.id, accessToken)
  }

  /** 转图与发布解耦：手动把该发布包的转图事项重新排队到后台，处理完会有站内消息反馈。 */
  async function retryReleasePreview(releasePackageId: string) {
    operationPending.value = true
    operationError.value = ''
    try {
      const result = await retryReleasePreviewRequest(releasePackageId, accessToken)
      await reload()
      return result
    } catch (error) {
      operationError.value = messageFrom(error)
  ...
  async function updateProjectPhaseOwners(projectId: string, phaseOwners: ProjectPhaseOwners) {
    operationPending.value = true
    try {
      const saved = await updateProjectPhaseOwnersRequest(projectId, phaseOwners, accessToken)
      await reload(project.value.id)
      return saved
    } finally { operationPending.value = false }
  }
  ...
  return {
    ...
    decideDrawingReviewSupervisor,
    resubmitDrawingReviewItem,
    createPackage,
    updatePackageDraft,
    deletePackageDraft,
    uploadPackageFile,
