...
import U9SyncBlockerDrawer from './components/U9SyncBlockerDrawer.vue'
...
// 默认展开审核栏，与网页端/客户端保持一致；用户手动折叠后按用户选择。
const drawingReviewPanelCollapsed = ref(false)
...
// 三类BOM明细编辑权限按专业拆分（标准件/非标件→机械权限，电气→电气权限），可在角色权限设置中配置。
const bomEditable = computed(() => activeBomKind.value === 'Electrical'
  ? workspace.hasPermission('bom.electrical.edit')
  : workspace.hasPermission('bom.mechanical.edit'))
...
const canManageProjectPlan = computed(() => {
  if (workspace.hasRole('developer')) return true
  const username = workspace.currentUsername.value.trim().toLocaleLowerCase()
  const project = workspace.project.value
  return project.primaryProjectManager?.trim().toLocaleLowerCase() === username
})
const canManageProjectPlanTemplates = computed(() => ['Administrator', 'platform_admin', 'developer'].some(workspace.hasRole))
...
const releaseBlockerDrawerOpen = ref(false)
const releaseBlockerDrawerBlockers = ref<U9SyncBlocker[]>([])
let releaseBlockerResolve: ((confirmed: boolean) => void) | undefined

/** 发布前提醒：料号未就绪的子件发布后无法自动同步到U9C，用右侧抽屉列清单让发布人决定。 */
function askReleaseBlockerConfirmation(blockers: U9SyncBlocker[]) {
  releaseBlockerDrawerBlockers.value = blockers
  releaseBlockerDrawerOpen.value = true
  return new Promise<boolean>(resolve => { releaseBlockerResolve = resolve })
}

function settleReleaseBlockerConfirmation(confirmed: boolean) {
  const resolve = releaseBlockerResolve
  releaseBlockerResolve = undefined
  releaseBlockerDrawerBlockers.value = []
  releaseBlockerDrawerOpen.value = false
  resolve?.(confirmed)
}

// 抽屉被点了关闭/遮罩关闭时，按"取消"处理，避免提交动作悬着。
watch(releaseBlockerDrawerOpen, open => {
  if (!open && releaseBlockerResolve) settleReleaseBlockerConfirmation(false)
})

async function submitReleasePackage(releasePackageId: string) {
  const blockers = await workspace.listU9SyncBlockers().catch(() => [] as U9SyncBlocker[])
  if (blockers.length && !(await askReleaseBlockerConfirmation(blockers))) return
  await runOperation(() => workspace.submitPackage(releasePackageId), '发布包已提交审批')
}
...
const drawingReviewStates = computed<Record<string, DrawingReviewBadge>>(() => {
  ...
  const archivedDocumentIds = new Set(workspace.managedDocuments.value
  ...
  for (const documentId of nonStandardModelIds) states[documentId] = initialReviewBadge(documentId)
  for (const relation of workspace.documentRelations.value) {
    if (nonStandardModelIds.has(relation.modelDocumentId)) states[relation.drawingDocumentId] = initialReviewBadge(relation.drawingDocumentId)
  }
...
const canAddProjectManagerNote = computed(() => {
  if (workspace.hasRole('Administrator') || workspace.hasRole('platform_admin') || workspace.hasRole('developer')) return true
  const username = workspace.currentUsername.value.trim().toLocaleLowerCase()
  const project = workspace.project.value
  const root = project.parentProjectId ? workspace.projects.value.find(item => item.id === (project.rootProjectId ?? project.parentProjectId)) : project
  return [project, root].some(item => item && [item.primaryProjectManager, ...(item.collaborativeProjectManagers ?? [])]
    .some(candidate => candidate?.trim().toLocaleLowerCase() === username))
})
...
const drawingReviewOverlayState = computed(() => ({
  ...
  reviewerOptions: drawingReviewReviewerOptions.value,
...
async function openProjectTab(tab: ProjectTab) {
  // 发布页与BOM页共用同一个面板：发布页签让面板切到发布视图，BOM页签切回普通BOM视图。
  const releaseRequested = tab === 'release'
  let panelTab: ProjectTab = releaseRequested ? 'bom' : tab
  if (projectTab.value === 'bom' && panelTab !== 'bom' && bomHasUnsavedChanges.value) {
    ElMessage.warning('当前BOM有未保存修改，请先保存或撤销修改后再离开。')
    return
  }
  if (!workspace.project.value.canReadContent && panelTab !== 'overview' && panelTab !== 'records') panelTab = 'overview'
  const nextTab: ProjectTab = releaseRequested && panelTab === 'bom' ? 'release' : panelTab
  if (panelTab === 'bom') mountedBomProjectId.value = workspace.project.value.id
  if (panelTab === 'documents') mountedDocumentsProjectId.value = workspace.project.value.id
...
async function openProjectTabFromHeader(tab: ProjectTab) {
  ...
  await openProjectTab(tab)
}

// BOM面板切换视图时同步顶部页签：发布视图高亮“发布”，切回其它视图高亮“BOM”。
function handleBomKindChange(kind: string) {
  activeBomKind.value = kind
  if (kind === 'Release') {
    if (projectTab.value === 'bom') projectTab.value = 'release'
    return
  }
  if (projectTab.value === 'release') projectTab.value = 'bom'
}
...
async function openReleasePackage(projectId: string, releasePackageId: string) {
  requestedReleasePackageId.value = ''
  if (await openManagedProject(projectId, 'bom')) {
...
async function openHierarchyBom(projectId: string, kind: Exclude<import('./types').BomKind, 'Unclassified'>) {
  requestedBomKind.value = undefined
  if (await openManagedProject(projectId, 'bom')) requestedBomKind.value = kind
}
...
async function openNotification(notification: UserNotification) {
  ...
  if (notification.category === 'ProjectTodo' && notification.projectId) {
    await openManagedProject(notification.projectId, 'overview')
    return
  }
  ...
  if (notification.projectId && notification.releasePackageId) {
    await openReleasePackage(notification.projectId, notification.releasePackageId)
  }
}
...
<template>
  <div class="pdm-app-shell" :class="`theme-${theme}`">
        ...
        <section v-else-if="activeView === 'workspace'" class="pdm-project-workspace">
          <ProjectWorkspaceHeader :project="workspace.project.value" :projects="workspace.projects.value" :active-tab="projectTab" :active-project-document-status="activeProjectDocumentStatus" :active-document-counts="workspace.documentFilterCounts.value" :current-username="workspace.currentUsername.value" :switching-project-id="switchingProjectId" :token="workspace.getAccessToken()" :can-copy-content="workspace.hasPermission('project.create')" :can-reset-content="workspace.hasPermission('project.content.reset')" :pending="workspace.operationPending.value" :on-preview-project-copy="workspace.previewProjectCopy" :on-copy-project-content="workspace.copyProjectContent" :on-content-reset-complete="() => workspace.reload(workspace.project.value!.id)" @back="openProjectList" @switch="switchProject" @tab="openProjectTabFromHeader">
            ...
            <WorkbenchHome
              ...
              :pending="workspace.operationPending.value"
              :token="workspace.getAccessToken()"
              :can-add-manager-note="canAddProjectManagerNote"
              :on-update-main-staffing="workspace.updateMainProjectStaffing"
              :on-update-designers="workspace.updateChildProjectDesigners"
                  ...
                  <div class="pdm-preview-layout" :class="{ 'is-desktop-preview': desktopAvailable }">
                    <PreviewWorkspace :selected="workspace.selectedNode.value" :related="workspace.relatedNodes.value" :bom-item="workspace.selectedBomItem.value" :current-username="workspace.currentUsername.value" :can-manage-lifecycle="workspace.hasPermission('release.manage')" :can-edit-documents="workspace.hasPermission('document.edit')" :desktop-available="desktopAvailable" :access-token="workspace.getAccessToken()" :project-id="workspace.project.value.id" :active="projectTab === 'documents'" :obscured="workspace.versionDrawerOpen.value || workspace.whereUsedDrawerOpen.value" :review-panel-collapsed="drawingReviewPanelCollapsed" :review-version-id="selectedDrawingReviewVersionId" :review-revision="selectedDrawingReviewRevision" :review-editable="selectedDrawingReviewItem?.drawingState === 'ChangesRequested'" :can-writeback-review-properties="canWritebackSelectedDrawingReview" @open="workspace.openDocument" @preview="workspace.previewDocument" @related="workspace.selectRelatedNode" @more="workspace.openVersionDrawer()" @where-used="workspace.openWhereUsed" @obsolete="obsoleteSelectedDocument">
                      <DrawingReviewPanel
                        ...
                        :packages="workspace.drawingReviews.value"
                        :candidates="workspace.drawingReviewCandidates.value"
                        :reviewer-options="drawingReviewReviewerOptions"
    ...
    <U9SyncBlockerDrawer
      v-model="releaseBlockerDrawerOpen"
      :blockers="releaseBlockerDrawerBlockers"
      title="发布前提醒：U9C 料号未就绪"
      decision
      @decide="settleReleaseBlockerConfirmation"
    />
  </div>
</template>
