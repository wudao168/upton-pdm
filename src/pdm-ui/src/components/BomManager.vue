...
const emit = defineEmits<{
  ...
  batchPermanentlyDeleteManual: [itemIds: string[]]
...
const bomColumnSettingsOpen = ref(false)
const visibleBomColumnKeys = ref<BomColumnKey[]>([])
const relationLoading = ref(false)
const relationApplying = ref(false)
const selectedRelationMainId = ref('')
...
const relationReviewComplete = computed(() => !hasStagedEdits.value && !relationLoading.value && relationCompleteness.value !== null
  && currentRelationChecks.value.every(main => relationMainReviewed(main)))
const relationCurrentIncompleteCount = computed(() => currentRelationChecks.value
  .reduce((count, main) => count + main.groups.filter(group => !relationGroupReviewed(main.mainBomItemId, group)).length, 0))
const relationReviewTitle = computed(() => hasStagedEdits.value ? 'BOM已修改，请保存后重新核对'
  : relationLoading.value ? '正在检查关联物料核对状态'
    : relationCompleteness.value && currentRelationChecks.value.length === 0 ? '当前BOM暂无已发布关联配置'
      : relationReviewComplete.value ? '已核对'
        : relationCompleteness.value ? `${relationCurrentIncompleteCount.value}项待核对` : '尚未核对，请打开关联物料进行核对')
...
const relationCheckByMainId = computed(() => new Map((relationCompleteness.value?.mainMaterials ?? [])
  .map(main => [main.mainBomItemId, main] as const)))
const relationMainRows = computed(() => kind.value === 'Standard' || kind.value === 'Electrical'
  ? currentCategoryRows.value.filter(item => !!item.id && !item.pendingRemoval && relationCheckByMainId.value.has(item.id ?? ''))
  : [])
const currentRelationChecks = computed(() => relationMainRows.value
  .map(item => relationCheckByMainId.value.get(item.id ?? ''))
  .filter((item): item is MaterialRelationCompleteness['mainMaterials'][number] => !!item))
const selectedRelationMainRow = computed(() => relationMainRows.value.find(item => item.id === selectedRelationMainId.value))
const selectedRelationMainCheck = computed(() => relationCheckByMainId.value.get(selectedRelationMainId.value))
...
const recycleBinManualSelectedIds = computed(() => {
  const manualIds = new Set(recycleBinRows.value.flatMap(item => !item.sourceDocumentId && item.id ? [item.id] : []))
  return recycleBinSelectedIds.value.filter(itemId => manualIds.has(itemId))
})
...
function compareBomRows(current: EditableBomRow[], previous: BomItem[], mode: BomDisplayMode) {
  ...
  currentRows.forEach(({ item, key }) => {
    ...
    if (changes.length) {
      // 只有“版本”变化、其它字段完全一致时归为版本更新，避免零件升版把整张BOM都算成修改。
...
function drawingSlots(row: BomItem): BomDrawingSlot[] {
  const slots: BomDrawingSlot[] = [
    { kind: '2D', title: '尚未关联2D工程图' },
    { kind: '3D', title: '尚未关联3D模型' },
  ]
  if (!row.sourceDocumentId) return slots
  const sourceDocument = props.documents.find(document => document.id === row.sourceDocumentId)
  if (!sourceDocument) return slots
  const modelDocumentId = sourceDocument.kind === 'Drawing'
    ? props.documentRelations.find(relation => relation.drawingDocumentId === sourceDocument.id)?.modelDocumentId
    : sourceDocument.id
  const drawingDocumentId = sourceDocument.kind === 'Drawing'
    ? sourceDocument.id
    : props.documentRelations.find(relation => relation.modelDocumentId === sourceDocument.id)?.drawingDocumentId
  const drawing = props.documents.find(document => document.id === drawingDocumentId && document.kind === 'Drawing')
  const model = props.documents.find(document => document.id === modelDocumentId && document.kind !== 'Drawing')
  return slots.map(slot => {
    const document = slot.kind === '2D' ? drawing : model
    if (!document) return slot
    return documentIsReleased(document)
      ? { ...slot, document, released: true }
      : { ...slot, document, title: `${document.drawingNumber} 尚未发布，暂不能下载` }
  })
}

function drawingLinks(row: BomItem): BomDrawingLink[] {
  return drawingSlots(row)
    .filter((slot): slot is BomDrawingSlot & { document: ManagedDocument } => Boolean(slot.document && slot.released))
...
function previewFileName(link: BomDrawingLink) {
  const baseName = link.document.fileName.replace(/\.[^.]+$/, '') || link.document.drawingNumber || link.document.name
...
async function downloadDrawing(link: BomDrawingLink) {
  ...
  try {
    const versions = await listDocumentVersions(link.document.id, props.token)
    const expectedFormat = link.kind === '2D' ? 'Pdf' : 'Step'
    const version = versions.find(candidate => (candidate.status === 'Released' || candidate.status === 1)
      && candidate.preview
      && normalizedPreviewFormat(candidate.preview.format) === expectedFormat)
    if (!version) {
    ...
    downloadingDrawingIds.value = next
  }
}
...
function applyBatchInputToRows(input: BatchUpdateBomItemsInput) {
  ...
  rows.value.filter(row => operationItemIds(row).some(id => itemIds.has(id))).forEach(row => {
    input.fields.forEach(field => {
      ...
      else if (field === 'heatTreatment' && input.heatTreatment !== undefined) row.heatTreatment = input.heatTreatment
...
async function loadRelationCompleteness(showError = true) {
  ...
  try {
    ...
    if (sequence !== relationLoadSequence) return null
...
async function remindRelationsAfterBomSave() {
  const result = await loadRelationCompleteness(false)
  if (result?.incompleteGroupCount)
    ElMessage.warning(`BOM已保存；另有 ${result.incompleteGroupCount} 个关联物料组待核对，不影响后续保存或发布`)
}

async function openMaterialRelations() {
  if (kind.value !== 'Standard' && kind.value !== 'Electrical') return
  ...
  const availableIds = new Set(relationMainRows.value.map(item => item.id ?? ''))
  const selectedBomItemId = selectedIds.value.find(id => availableIds.has(id))
  if (selectedBomItemId) selectedRelationMainId.value = selectedBomItemId
  else if (!availableIds.has(selectedRelationMainId.value))
    selectedRelationMainId.value = currentRelationChecks.value[0]?.mainBomItemId ?? relationMainRows.value[0]?.id ?? ''
}
...
function relationMainStatus(mainBomItemId: string) {
  const main = relationCheckByMainId.value.get(mainBomItemId)
  return !main ? '未配置' : relationMainReviewed(main) ? '已核对' : '待核对'
}

function toggleRelationOption(mainBomItemId: string, group: MaterialRelationGroupCheck, optionId: string) {
  if (isSingleRelationGroup(group)) {
    updateRelationSingle(mainBomItemId, group.groupId, optionId)
    return
  }
  const selected = new Set(relationChoice(mainBomItemId, group.groupId))
  if (selected.has(optionId)) selected.delete(optionId)
  else selected.add(optionId)
  updateRelationMultiple(mainBomItemId, group.groupId, [...selected])
}

async function applySelectedRelations() {
  if (!props.projectId || !props.token || !currentRelationChecks.value.length) return
  relationApplying.value = true
  try {
    const payload = currentRelationChecks.value.map(main => ({
    ...
    if (!relationCheckByMainId.value.has(selectedRelationMainId.value))
      selectedRelationMainId.value = currentRelationChecks.value[0]?.mainBomItemId ?? relationMainRows.value[0]?.id ?? ''
...
function configurableBomView(): ConfigurableBomView | undefined {
  return kind.value === 'Standard' || kind.value === 'NonStandard' || kind.value === 'Electrical' || kind.value === 'WearPart' ? kind.value : undefined
}

function defaultBomColumnKeys(view: ConfigurableBomView): BomColumnKey[] {
  return bomColumnDefinitions.flatMap(column => {
    if (column.key === 'impact' && view === 'WearPart') return []
    if (column.key === 'heatTreatment' && view !== 'NonStandard' && view !== 'WearPart') return []
    // 标准件/电气件不转 STEP/PDF，这两类BOM不显示图纸列。
    if (column.key === 'drawing' && (view === 'Standard' || view === 'Electrical')) return []
    return [column.key]
  })
}

function availableBomColumns(view = configurableBomView()) {
  // 源数据视图也不显示图纸列：标准件/电气件不转 STEP/PDF，源数据里看图档来源用图档页签即可。
  if (!view) return bomColumnDefinitions.filter(column => column.key !== 'impact' && column.key !== 'heatTreatment' && column.key !== 'drawing')
  return bomColumnDefinitions.filter(column => (column.key !== 'impact' || view !== 'WearPart')
    && (column.key !== 'drawing' || (view !== 'Standard' && view !== 'Electrical')))
}

function bomColumnStorageKey(view: ConfigurableBomView) {
  return `upton-pdm:bom-columns:${props.username || 'anonymous'}:${view}`
}

function restoreBomColumns() {
  const view = configurableBomView()
  if (!view) {
    visibleBomColumnKeys.value = availableBomColumns().map(column => column.key)
    return
  }
  visibleBomColumnKeys.value = defaultBomColumnKeys(view)
  try {
    const saved = JSON.parse(window.localStorage.getItem(bomColumnStorageKey(view)) ?? 'null') as { visible?: BomColumnKey[] } | null
    const allowed = new Set(availableBomColumns(view).map(column => column.key))
    const visible = (saved?.visible ?? []).filter((key, index, values) => allowed.has(key) && values.indexOf(key) === index)
    if (visible.length) visibleBomColumnKeys.value = visible
  } catch { /* 个人列设置不可读时使用当前BOM类型默认列。 */ }
}

function isBomColumnVisible(key: BomColumnKey) {
  if (kind.value === 'Source') return key !== 'impact' && key !== 'drawing'
  return visibleBomColumnKeys.value.includes(key)
}

function saveBomColumns() {
  const view = configurableBomView()
  if (!view || !visibleBomColumnKeys.value.length) {
    ElMessage.warning('请至少保留一列')
    return
  }
  window.localStorage.setItem(bomColumnStorageKey(view), JSON.stringify({ visible: visibleBomColumnKeys.value }))
  bomColumnSettingsOpen.value = false
  ElMessage.success('BOM列设置已保存到当前账号')
}

function resetBomColumns() {
  const view = configurableBomView()
  if (view) visibleBomColumnKeys.value = defaultBomColumnKeys(view)
}

function visibleBomColumnCount() {
  return bomColumnDefinitions.filter(column => isBomColumnVisible(column.key)).length
}
...
watch(() => props.username, restoreBomColumns)
...
async function permanentlyDeleteManualRecycleBin() {
  const itemIds = [...recycleBinManualSelectedIds.value]
  if (itemIds.length === 0 || props.pending) return
  try {
    await ElMessageBox.confirm(
      `将永久删除所选 ${itemIds.length} 条人工来源物料，删除后无法恢复。所选有图档来源的物料不会删除，仍保留在回收站。`,
      '确认彻底删除',
      { confirmButtonText: '彻底删除', cancelButtonText: '取消', type: 'error', closeOnClickModal: false },
    )
    emit('batchPermanentlyDeleteManual', itemIds)
    recycleBinSelectedIds.value = recycleBinSelectedIds.value.filter(itemId => !itemIds.includes(itemId))
  } catch {
    // 用户取消时不修改BOM。
  }
}
...
async function commitInlineEdit(row: BomItem) {
  ...
  if (edit.field === 'heatTreatment') input.heatTreatment = String(value)
  ...
  <section class="pdm-panel pdm-manager-panel pdm-bom-manager-panel" aria-label="BOM维护">
    ...
    <section v-if="isReleaseView" class="pdm-bom-release-overview" aria-label="发布管理">
      <ReleaseOverview
        ...
        :token="token"
        :can-manage-release="canManageRelease"
    ...
    <div v-if="canSelectCurrentView || !isSourceView" class="pdm-bom-selection-toolbar">
      ...
      <div class="pdm-bom-filters" role="search" aria-label="筛选BOM物料">
        ...
        <button v-if="configurableBomView()" type="button" class="pdm-secondary-action pdm-bom-column-settings-action" @click="bomColumnSettingsOpen = true">列设置</button>
      </div>
      <div v-if="!isSourceView && !isWearPartView" class="pdm-bom-version-picker">
        ...
        <button type="button" class="pdm-secondary-action" :disabled="!comparisonBaseline" @click="comparisonOpen = !comparisonOpen">{{ comparisonOpen ? '收起差异' : '对比发布' }}</button>
      </div>
    </div>
      ...
      <table class="pdm-edit-table pdm-bom-table">
        ...
        <tbody>
          <tr v-for="{ row, index, depth, hasChildren, expanded, structureKey } in pagedRows" :key="row.id || row._clientKey" :data-row-index="index" :title="comparisonRowTitle(row)" :class="{ 'is-quick-entry': row._quickEntry, 'is-pending-removal': row.pendingRemoval, 'is-release-excluded': row.releaseExcluded, 'is-bom-unresolved': !row._quickEntry && (rowNeedsClassification(row) || row.manualUnmatched), 'is-data-exception': !row._quickEntry && dataStatusIssues(row).length > 0, 'is-reconciliation-issue': !row._quickEntry && shouldShowReconciliation(row), 'is-release-unchanged': comparisonRowStatus(row) === 'Released', 'is-release-added': comparisonRowStatus(row) === 'Added', 'is-release-version-updated': comparisonRowStatus(row) === 'VersionUpdated', 'is-release-property-updated': comparisonRowStatus(row) === 'PropertyUpdated', 'is-release-modified': comparisonRowStatus(row) === 'Modified', 'is-row-dragging': draggedRowIndex === index, 'is-drag-over-before': dragOverRowIndex === index && dragOverPosition === 'before', 'is-drag-over-after': dragOverRowIndex === index && dragOverPosition === 'after' }">
            ...
            <td v-if="isBomColumnVisible('kind')">
            ...
            <td v-if="isBomColumnVisible('wearPart')" class="pdm-bom-wear-part-cell">
            ...
            <td v-if="isBomColumnVisible('impact') && !isSourceView && !isWearPartView" class="pdm-bom-impact-cell">
            ...
            <td v-if="isBomColumnVisible('unit')"><span class="pdm-bom-cell-value">{{ u9UnitName(row.unit || '001') }}</span></td>
            <td v-if="isBomColumnVisible('drawingNumber')" :class="{ 'pdm-bom-structure-code-cell': displayMode === 'Structure' }">
            ...
            <td v-if="isBomColumnVisible('name')">
            ...
            <td v-if="isBomColumnVisible('brand')">
            ...
            <td v-if="isBomColumnVisible('material')">
            ...
            <td v-if="isBomColumnVisible('surfaceTreatment')">
            ...
            <td v-if="isBomColumnVisible('heatTreatment')">
              <input v-if="isInlineEditing(row, 'heatTreatment')" v-model="inlineValue" class="pdm-bom-inline-editor" aria-label="内联编辑热处理" autofocus @blur="commitInlineEdit(row)" @keydown.enter.prevent="commitInlineEdit(row)" @keydown.esc.prevent="cancelInlineEdit">
              <button v-else-if="row.id && canEditCurrentView" type="button" class="pdm-bom-cell-edit" :title="row.heatTreatment || '点击编辑热处理'" aria-label="编辑热处理" @click="beginInlineEdit(row, 'heatTreatment')">{{ displayValue(row.heatTreatment) }}</button>
              <input v-else-if="canEditCurrentView && !isSourceView" v-model.trim="row.heatTreatment" aria-label="热处理">
              <span v-else class="pdm-bom-cell-value" :title="row.heatTreatment">{{ displayValue(row.heatTreatment) }}</span>
            </td>
            <td v-if="isBomColumnVisible('weight')">
            ...
            <td v-if="isBomColumnVisible('quantity')" class="pdm-bom-quantity-audit">
            ...
            <td v-if="isBomColumnVisible('quantityReference')" class="pdm-bom-quantity-reference" :aria-label="row._quickEntry ? undefined : isWearPartView ? '统计清单，不参与发布' : quantityReferenceTitle(row)" :title="row._quickEntry ? undefined : isWearPartView ? '统计清单，不参与发布' : quantityReferenceTitle(row)">
            ...
            <td v-if="isBomColumnVisible('dataStatus') && row._quickEntry" class="pdm-bom-data-status-cell"><span class="pdm-bom-data-status">待录入</span></td>
            <td v-else-if="isBomColumnVisible('dataStatus')" class="pdm-bom-data-status-cell" :class="dataStatusIssues(row).length ? 'is-incomplete' : row.releaseExcluded ? 'is-release-excluded' : 'is-complete'"><span class="pdm-bom-data-status" :class="dataStatusIssues(row).length ? 'is-incomplete' : row.releaseExcluded ? 'is-release-excluded' : 'is-complete'" :title="row.releaseExcluded ? `不发布：${row.releaseExclusionReason || '未记录原因'}` : dataStatusIssues(row).length ? `待完善：${dataStatusIssues(row).join('、')}` : '必填资料及料品主档校验均已通过'">{{ dataStatusLabel(row) }}</span></td>
          </tr>
          <tr v-if="filteredRows.length === 0 && !canShowQuickEntry"><td :colspan="3 + visibleBomColumnCount()" class="pdm-empty-info">{{ rows.length ? '没有符合当前筛选条件的物料。' : isWearPartView ? '当前没有标记为易损件的物料。' : isSourceView ? '当前没有图档源数据。' : '当前BOM为空，系统自动按无此类物料处理；可新增物料或导入标准XLSX。' }}</td></tr>
        </tbody>
      </table>
    </div>
    ...
    <el-dialog v-model="bomColumnSettingsOpen" title="BOM列设置" width="520px" append-to-body>
      ...
      <el-checkbox-group v-model="visibleBomColumnKeys" class="pdm-bom-column-settings-list" aria-label="BOM显示列">
        <el-checkbox v-for="column in availableBomColumns()" :key="column.key" :value="column.key">{{ column.label }}</el-checkbox>
      </el-checkbox-group>
      <template #footer>
        <el-button @click="resetBomColumns">恢复默认</el-button>
        <el-button @click="bomColumnSettingsOpen = false">取消</el-button>
        <el-button type="primary" @click="saveBomColumns">保存</el-button>
      </template>
    </el-dialog>

    <el-drawer v-model="relationDialogOpen" class="pdm-material-relation-drawer" title="BOM 关联物料核对" size="min(1360px, 96vw)" :close-on-click-modal="false" destroy-on-close>
      <div class="pdm-material-relation-help">系统只提醒、不自动加入，也不会阻止保存或发布。标准件和电气件分别按当前 BOM 核对；相同配件仍按主物料行独立关联和计数。</div>
      <div v-if="relationLoading" class="pdm-empty-info pdm-material-relation-loading">正在核对关联配置与 BOM…</div>
      <div v-else class="pdm-material-relation-workspace">
        <section class="pdm-material-relation-pane" aria-label="主物料明细">
          <header><strong>主物料明细</strong><span>共 {{ relationMainRows.length }} 条</span></header>
          <div class="pdm-material-relation-table-wrap">
            <table class="pdm-edit-table pdm-material-relation-main-table">
              <thead><tr><th>物料编码</th><th>物料名称</th><th>型号</th><th>数量</th><th>状态</th></tr></thead>
              <tbody>
                <tr v-for="main in relationMainRows" :key="main.id" :class="{ 'is-selected': main.id === selectedRelationMainId }" tabindex="0" @click="selectedRelationMainId = main.id ?? ''" @keydown.enter="selectedRelationMainId = main.id ?? ''">
                  <td :title="main.drawingNumber">{{ main.drawingNumber || '—' }}</td><td :title="main.name">{{ main.name || '—' }}</td><td :title="main.specification">{{ main.specification || '—' }}</td><td>{{ main.quantity }} {{ u9UnitName(main.unit) }}</td><td><span class="pdm-material-relation-status" :class="`is-${relationMainStatus(main.id ?? '')}`">{{ relationMainStatus(main.id ?? '') }}</span></td>
                </tr>
                <tr v-if="!relationMainRows.length"><td colspan="5" class="pdm-empty-info">当前 BOM 没有已发布关联配置的主物料。</td></tr>
              </tbody>
            </table>
          </div>
        </section>
        <section class="pdm-material-relation-pane" aria-label="关联物料明细">
          <header><strong>关联物料明细</strong><span v-if="selectedRelationMainCheck">配置 V{{ selectedRelationMainCheck.revisionVersion }}</span></header>
          <div v-if="!selectedRelationMainRow" class="pdm-material-relation-empty"><strong>请选择主物料</strong><span>选中左侧主物料后，这里显示对应的关联物料。</span></div>
          <div v-else-if="!selectedRelationMainCheck" class="pdm-material-relation-empty"><strong>暂无关联物料配置</strong><span>该主物料尚未发布关联配置。</span></div>
          <div v-else class="pdm-material-relation-groups">
            <section v-for="group in selectedRelationMainCheck.groups" :key="group.groupId" class="pdm-material-relation-group" :class="{ 'is-incomplete': !relationGroupReviewed(selectedRelationMainCheck.mainBomItemId, group) }">
              <div class="pdm-material-relation-group-title"><div><strong>{{ group.groupName }}</strong><span>{{ group.isRequired ? '需核对' : '可选参考' }} · {{ isSingleRelationGroup(group) ? '只能选 1 项' : `可同时选多项（最多 ${group.maxSelection ?? group.options.length} 项）` }}</span></div><div class="pdm-material-relation-group-actions"><div v-if="group.isRequired" class="pdm-material-relation-no-accessory"><span>{{ relationNoAccessory(selectedRelationMainCheck.mainBomItemId, group.groupId) ? '已确认本次无需配套' : '未确认无需配套' }}</span><input :value="relationNoAccessoryReason(selectedRelationMainCheck.mainBomItemId, group.groupId)" maxlength="500" placeholder="补充原因（选填）" :aria-label="`${selectedRelationMainCheck.mainMaterialCode} ${group.groupName} 无需配套原因`" :disabled="!relationNoAccessory(selectedRelationMainCheck.mainBomItemId, group.groupId)" @input="updateRelationNoAccessoryReason(selectedRelationMainCheck.mainBomItemId, group.groupId, ($event.target as HTMLInputElement).value)"><button type="button" class="pdm-secondary-action pdm-material-relation-group-button pdm-material-relation-review-button" :disabled="!relationNoAccessory(selectedRelationMainCheck.mainBomItemId, group.groupId)" @click="reopenRelationGroup(selectedRelationMainCheck.mainBomItemId, group.groupId)">重新核对</button></div><button v-if="group.isRequired" type="button" class="pdm-material-relation-none-button pdm-material-relation-group-button" :disabled="relationNoAccessory(selectedRelationMainCheck.mainBomItemId, group.groupId)" @click="confirmRelationNoAccessory(selectedRelationMainCheck.mainBomItemId, group.groupId)">无需配套</button><button type="button" class="pdm-secondary-action pdm-material-relation-clear-button pdm-material-relation-group-button" :disabled="!relationChoice(selectedRelationMainCheck.mainBomItemId, group.groupId).length" @click="updateRelationChoice(selectedRelationMainCheck.mainBomItemId, group.groupId, [])">清除选择</button><em>{{ relationGroupStatus(selectedRelationMainCheck.mainBomItemId, group) }}</em></div></div>
              <div class="pdm-material-relation-option-list">
                <table class="pdm-edit-table" aria-label="关联物料候选列表">
                  <thead><tr><th>选择</th><th>名称</th><th>型号</th><th>品牌</th><th>选型建议</th><th>数量</th></tr></thead>
                  <tbody>
                    <tr v-for="option in group.options" :key="option.id" :class="{ 'is-selected': relationChoice(selectedRelationMainCheck.mainBomItemId, group.groupId).includes(option.id), 'is-preferred': option.isDefault }" @click="toggleRelationOption(selectedRelationMainCheck.mainBomItemId, group, option.id)">
                      <td><input :type="isSingleRelationGroup(group) ? 'radio' : 'checkbox'" :name="`relation-${selectedRelationMainCheck.mainBomItemId}-${group.groupId}`" :checked="relationChoice(selectedRelationMainCheck.mainBomItemId, group.groupId).includes(option.id)" :aria-label="`选择关联物料 ${option.materialCode}`" @click.stop @change="toggleRelationOption(selectedRelationMainCheck.mainBomItemId, group, option.id)"></td>
                      <td :title="option.materialName"><span v-if="option.isDefault" class="pdm-material-relation-preferred">优先</span>{{ option.materialName || '—' }}</td><td :title="option.specification || ''">{{ option.specification || '—' }}</td><td :title="option.brand || ''">{{ option.brand || '—' }}</td><td :title="option.selectionAdvice || ''">{{ option.selectionAdvice || '—' }}</td><td :title="relationQuantityText(group, option.id, selectedRelationMainCheck.mainQuantity)">{{ relationQuantityText(group, option.id, selectedRelationMainCheck.mainQuantity) }}</td>
                    </tr>
                  </tbody>
                </table>
              </div>
      ...
      <section class="pdm-bom-batch-dialog pdm-bom-recycle-dialog" role="dialog" aria-modal="true" aria-labelledby="pdm-bom-recycle-title">
        ...
        <footer class="pdm-bom-recycle-footer">
          <div class="pdm-bom-recycle-help">
            ...
            <span>彻底删除仅适用于人工来源项，删除后无法恢复。</span>
          </div>
          <button type="button" class="pdm-secondary-action is-danger" :disabled="pending || recycleBinManualSelectedIds.length === 0" @click="permanentlyDeleteManualRecycleBin">彻底删除人工项（{{ recycleBinManualSelectedIds.length }}）</button>
...
.pdm-bom-manager-panel .pdm-bom-kind[data-kind="NonStandard"]{color:var(--pdm-purple)}
.pdm-bom-manager-panel .pdm-bom-kind[data-kind="Electrical"]{color:var(--pdm-orange)}
.pdm-bom-manager-panel .pdm-bom-kind[data-kind="Virtual"]{color:var(--pdm-text-soft)}
...
.pdm-bom-manager-panel :deep(.pdm-bom-table tbody tr.is-release-unchanged>.pdm-bom-quantity-reference){background:#f0fdf4}.pdm-bom-manager-panel :deep(.pdm-bom-table tbody tr.is-release-added>td){background:#fff0a8}.pdm-bom-manager-panel :deep(.pdm-bom-table tbody tr.is-release-added:hover>td){background:#ffe477}.pdm-bom-manager-panel :deep(.pdm-bom-table tbody tr.is-release-modified>td){background:#fff0a8}.pdm-bom-manager-panel :deep(.pdm-bom-table tbody tr.is-release-modified:hover>td){background:#ffe477}
.pdm-bom-comparison-filters{display:flex;align-items:center;gap:5px;flex-wrap:wrap}.pdm-bom-comparison-filters button{min-height:24px;padding:2px 8px;border:1px solid var(--pdm-border);border-radius:999px;background:#fff;color:var(--pdm-muted);font-size:11px;cursor:pointer}.pdm-bom-comparison-filters button.is-active{border-color:var(--pdm-blue);box-shadow:0 0 0 1px var(--pdm-blue);color:var(--pdm-text);font-weight:600}.pdm-bom-comparison-filters button.is-released{background:#f0fdf4;color:var(--pdm-green)}.pdm-bom-comparison-filters button.is-added{background:#eff6ff;color:#2563eb}.pdm-bom-comparison-filters button.is-version-updated{background:#eef4ff;color:#3b6fd4}.pdm-bom-comparison-filters button.is-modified{background:#fff7ed;color:var(--pdm-orange)}.pdm-bom-comparison-filters button.is-removed{background:#fef2f2;color:var(--pdm-danger)}
...
/* 所有相对最近正式版尚未发布的行统一用浅黄色突出显示。 */
.pdm-bom-comparison-filters button.is-property-updated{background:#f0f9ff;color:#0c7fb0}
.pdm-bom-manager-panel :deep(.pdm-bom-table tbody tr.is-release-version-updated>td){background:#fff0a8}
.pdm-bom-manager-panel :deep(.pdm-bom-table tbody tr.is-release-version-updated:hover>td){background:#ffe477}
.pdm-bom-manager-panel :deep(.pdm-bom-table tbody tr.is-release-property-updated>td){background:#fff0a8}
.pdm-bom-manager-panel :deep(.pdm-bom-table tbody tr.is-release-property-updated:hover>td){background:#ffe477}
</style>
<style scoped>
...
.pdm-material-relation-help{padding:9px 12px;border-bottom:1px solid #bfdbfe;background:#eff6ff;color:var(--pdm-blue);line-height:1.4}
...
.pdm-material-relation-pane{display:flex;min-width:0;min-height:0;flex-direction:column;overflow:hidden;border:1px solid var(--pdm-border);border-radius:7px;background:#fff}
.pdm-material-relation-pane>header{display:flex;min-height:42px;align-items:center;justify-content:space-between;gap:10px;padding:7px 10px;border-bottom:1px solid var(--pdm-border);background:#f8fafc}
.pdm-material-relation-pane>header>div{display:grid;min-width:0;gap:2px}.pdm-material-relation-pane>header span{overflow:hidden;color:var(--pdm-muted);text-overflow:ellipsis;white-space:nowrap}
.pdm-material-relation-table-wrap,.pdm-material-relation-groups{min-height:0;overflow-y:auto;overflow-x:hidden}.pdm-material-relation-table-wrap{flex:1}
.pdm-material-relation-main-table{width:100%;table-layout:fixed}.pdm-material-relation-drawer .pdm-edit-table th,.pdm-material-relation-drawer .pdm-edit-table td{text-align:center;vertical-align:middle}.pdm-material-relation-main-table th:nth-child(1){width:22%}.pdm-material-relation-main-table th:nth-child(2){width:25%}.pdm-material-relation-main-table th:nth-child(3){width:24%}.pdm-material-relation-main-table th:nth-child(4){width:15%}.pdm-material-relation-main-table th:nth-child(5){width:14%}
.pdm-material-relation-main-table tbody tr{height:30px;cursor:pointer}.pdm-material-relation-main-table tbody tr.is-selected td{background:var(--pdm-theme-accent-soft);color:var(--pdm-theme-accent)}.pdm-material-relation-main-table td{overflow:hidden;text-overflow:ellipsis;white-space:nowrap}
.pdm-material-relation-status{display:inline-flex;padding:1px 5px;border-radius:999px;background:#f1f5f9;color:var(--pdm-muted);white-space:nowrap}.pdm-material-relation-status.is-已核对{background:#f0fdf4;color:var(--pdm-green)}.pdm-material-relation-status.is-待核对{background:#fff7ed;color:var(--pdm-orange)}
.pdm-material-relation-empty{display:grid;min-height:180px;place-content:center;gap:7px;text-align:center}.pdm-material-relation-empty span{color:var(--pdm-muted)}
...
.pdm-material-relation-group-title{display:flex;align-items:center;justify-content:space-between;gap:8px}.pdm-material-relation-group-title>div:first-child{display:flex;min-width:0;align-items:center;gap:7px}.pdm-material-relation-group-title strong{flex:0 0 auto;white-space:nowrap}.pdm-material-relation-group-title span{min-width:0;overflow:hidden;color:var(--pdm-muted);font-size:11px;text-overflow:ellipsis;white-space:nowrap}.pdm-material-relation-group-actions{display:flex;min-width:0;flex:0 1 auto;align-items:center;gap:6px}.pdm-material-relation-group-title em{padding:1px 6px;border-radius:999px;background:#f1f5f9;color:var(--pdm-text-soft);font-size:11px;font-style:normal;white-space:nowrap}
.pdm-material-relation-option-list{max-width:100%;overflow:hidden;border:1px solid var(--pdm-border);border-radius:5px}.pdm-material-relation-option-list table{width:100%;min-width:0;table-layout:fixed}.pdm-material-relation-option-list th:first-child{width:48px}.pdm-material-relation-option-list th:nth-child(2){width:22%}.pdm-material-relation-option-list th:nth-child(3){width:18%}.pdm-material-relation-option-list th:nth-child(4){width:14%}.pdm-material-relation-option-list th:nth-child(5){width:25%}.pdm-material-relation-option-list th:nth-child(6){width:13%}.pdm-material-relation-option-list tbody tr{height:30px;cursor:pointer}.pdm-material-relation-option-list tbody tr.is-selected td{background:var(--pdm-theme-accent-soft)}.pdm-material-relation-option-list tbody tr.is-preferred td{background:#f0fdf4;color:var(--pdm-green)}.pdm-material-relation-option-list tbody tr.is-selected.is-preferred td{background:#dcfce7}.pdm-material-relation-option-list td{min-width:0;overflow:hidden;text-overflow:ellipsis;white-space:nowrap}.pdm-material-relation-option-list td:first-child input{position:relative;z-index:1;display:block;box-sizing:border-box;width:16px!important;min-width:16px!important;max-width:16px!important;height:16px!important;min-height:16px!important;max-height:16px!important;margin:0 auto;padding:0}.pdm-material-relation-preferred{display:inline-flex;margin-right:4px;padding:0 4px;border-radius:4px;background:#16a34a;color:#fff;font-size:11px;font-weight:600;line-height:16px;vertical-align:middle}
.pdm-material-relation-group-button{box-sizing:border-box;width:80px!important;min-width:80px!important;max-width:80px!important;height:30px!important;min-height:30px!important;max-height:30px!important;justify-content:center;padding:0 6px;white-space:nowrap}.pdm-material-relation-none-button{border:1px solid var(--pdm-danger);border-radius:5px;background:var(--pdm-danger);color:#fff;line-height:28px;cursor:pointer}.pdm-material-relation-none-button:hover,.pdm-material-relation-none-button:focus-visible{filter:brightness(.92);outline:2px solid color-mix(in srgb,var(--pdm-danger) 25%,transparent);outline-offset:1px}.pdm-material-relation-none-button:disabled{filter:none;opacity:.45;cursor:not-allowed}.pdm-material-relation-no-accessory{display:flex;min-width:0;align-items:center;gap:5px;color:var(--pdm-text-soft)}.pdm-material-relation-no-accessory>span{flex:0 0 auto}.pdm-material-relation-no-accessory input{box-sizing:border-box;width:110px;min-width:70px;height:30px;padding:3px 7px;border:1px solid var(--pdm-border);border-radius:5px;background:#fff;color:var(--pdm-text)}.pdm-material-relation-no-accessory input:disabled{background:#f8fafc;color:var(--pdm-muted)}
