...
<template>
  <section class="pdm-panel production-drawings" aria-label="生产图纸中心">
    <div class="production-drawings__filters">
      ...
      <select v-model="publisherFilter" aria-label="按发布人筛选"><option value="">全部发布人</option><option v-for="name in publishers" :key="name" :value="name">{{ name }}</option></select>
      ...
      <select v-model="managerFilter" aria-label="按项目经理筛选"><option value="">全部项目经理</option><option v-for="name in managers" :key="name" :value="name">{{ name }}</option></select>
      <label><input v-model="history" type="checkbox"> 查看历史正式版</label>
      <form @submit.prevent="locateScan"><input v-model="scan" placeholder="扫描图纸二维码" aria-label="扫描图纸二维码"><button type="submit">定位</button></form>
    </div>
...
@media(max-width:950px){.production-drawings__project{min-width:190px}.production-drawings__selection .production-drawings__toolbar-actions{margin-left:0}}
