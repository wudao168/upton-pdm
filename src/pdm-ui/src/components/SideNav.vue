...
function repairMojibake(value: string) {
  ...
  for (let attempt = 0; attempt < 3; attempt += 1) {
    ...
    const codePoints = Array.from(current, character => {
      const codePoint = character.charCodeAt(0)
      return codePoint <= 255 ? codePoint : windows1252Bytes.get(codePoint) ?? -1
    })
    if (codePoints.some(codePoint => codePoint < 0)) break
    try {
      const decoded = new TextDecoder('utf-8', { fatal: true }).decode(Uint8Array.from(codePoints))
      if (decoded === current || !/[\u3400-\u9fff]/u.test(decoded)) break
      current = decoded
    } catch {
      break
    }
