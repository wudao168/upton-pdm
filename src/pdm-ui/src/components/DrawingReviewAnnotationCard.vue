...
// 审核已经走完（主管已批准/写标记/版本冲突/撤销）后，结论栏要反映审核单的真实状态，而不是继续显示"等待批准"。
const finishedPackageLabels: Partial<Record<DrawingReviewPackage['state'], string>> = {
  Approved: '审核单已完成',
  WritingProperties: '正在写入审核标记',
  Stale: '但审核单版本冲突，需要重新发起审核',
  Withdrawn: '审核单已撤销',
}
const decisionRouteLabel = computed(() => {
  ...
  if (itemApproved.value) {
    if (supervisorApproval.value) return '该2D工程图已通过审核，等待批准；批准为整单操作，退回只作用于当前选中的图纸'
...
async function decide(decision: DrawingReviewDecision) {
  ...
  // 通过/批准不再二次确认；退回必须确认，并写明只退回当前这一张。
  if (decision !== 'Approve') {
...
function submit(decision: DrawingReviewDecision) {
  // 退回一律按当前选中的图纸处理；机械主管节点只有“批准”是整单操作。
...
async function revoke() {
  ...
  await ElMessageBox.confirm(
    ...
    itemApproved.value ? '撤销审核通过' : '撤销退回',
      ...
      <template v-else>
        ...
        <template v-else>
          ...
          <button
            ...
            type="button"
            class="is-revoke"
            :title="itemApproved ? '撤销审核通过' : '撤销退回结论'"
            :disabled="!canRevoke"
            @click="revoke()"
