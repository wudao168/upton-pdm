...
const emit = defineEmits<{ 'update:activeCompanyId': [companyId: string] }>()
...
const unitParent = computed(() => unitForm.parentUnitId ? companyUnits.value.find(unit => unit.id === unitForm.parentUnitId) : undefined)
const unitCodePrefix = computed(() => unitForm.kind === 'BusinessDivision' || !unitParent.value ? '' : `${unitParent.value.code.trim().toUpperCase()}-`)
const completeUnitCode = computed(() => `${unitCodePrefix.value}${localUnitCode(unitForm.code, unitParent.value)}`)
const unitCodeMaxLength = computed(() => Math.max(1, 40 - unitCodePrefix.value.length))
...
watch(() => props.activeCompanyId, companyId => { if (companyId && companyId !== currentCompanyId.value) currentCompanyId.value = companyId }, { immediate: true })

watch(currentCompanyId, companyId => {
  if (companyId) localStorage.setItem('pdm_active_organization', companyId)
  if (companyId && companyId !== props.activeCompanyId) emit('update:activeCompanyId', companyId)
...
function effectiveCompanyId(user: PdmUser) {
...
function managerCandidates() { return companyAssignableUsers.value }
...
function openUnit(item?: OrganizationUnit, parent?: OrganizationUnit) {
  ...
  const parentUnit = item?.parentUnitId ? props.directory.units.find(unit => unit.id === item.parentUnitId) : parent
  Object.assign(unitForm, item ? { ...item, code: localUnitCode(item.code, parentUnit), parentUnitId: item.parentUnitId, canManufacture: item.canManufacture === true } : {
    id: undefined,
    organizationId,
...
function localUnitCode(code: string, parent?: OrganizationUnit) {
  const normalized = code.trim().toUpperCase()
  if (!parent) return normalized
  const prefix = `${parent.code.trim().toUpperCase()}-`
  return normalized.startsWith(prefix) ? normalized.slice(prefix.length) : normalized
}
function nextSortOrder(parentUnitId?: string) {
  return companyUnits.value.filter(unit => unit.parentUnitId === parentUnitId).reduce((maximum, unit) => Math.max(maximum, unit.sortOrder), -1) + 1
}
...
async function saveUnit() {
  ...
  try {
    const saved = await props.onSaveUnit({ ...unitForm, code: completeUnitCode.value, parentUnitId: unitForm.kind === 'BusinessDivision' ? undefined : unitForm.parentUnitId, canManufacture: unitForm.kind === 'BusinessDivision' && unitForm.canManufacture })
...
<template>
  <section class="pdm-project-manager pdm-organization-settings">
    ...
    <template v-if="activeView === 'organization'">
      <section v-if="!embedded" class="pdm-panel org-company-switcher" aria-label="当前公司">
      ...
      <div class="org-workspace">
        ...
        <section class="pdm-panel org-detail-panel" aria-label="组织详情">
          <template v-if="showingUnassigned">
            <header class="org-detail-heading"><div><div class="org-detail-eyebrow">人员归属</div><h2>未分配人员</h2><p>仅显示主公司为当前公司、但尚未加入当前公司部门的账号。</p></div></header>
...
<style scoped>
.pdm-organization-settings { min-height: 0; flex: 1 1 auto; }
.org-embedded-heading { display: flex; align-items: center; justify-content: space-between; gap: 12px; margin-bottom: 10px; }.org-embedded-heading > div:first-child { display: grid; gap: 3px; }.org-embedded-heading strong { font-size: 14px; }.org-embedded-heading small { color: var(--pdm-muted); }
...
.org-company-codes { display: flex; justify-content: flex-end; gap: 22px; color: var(--pdm-muted); font-size: 11px; }.org-company-codes b { margin-left: 5px; color: var(--pdm-text); font-size: 12px; }
.org-workspace { min-height: 0; display: grid; flex: 1 1 auto; grid-template-columns: 300px minmax(0, 1fr); gap: 12px; }
.org-tree-panel, .org-detail-panel { min-width: 0; min-height: 0; padding: 15px; }
...
.org-tree-node { min-width: 0; display: inline-flex; align-items: center; gap: 7px; }.org-tree-node > span:nth-child(2) { overflow: hidden; text-overflow: ellipsis; white-space: nowrap; }.org-tree-node em { color: var(--pdm-danger); font-size: 11px; font-style: normal; }.org-tree-node em.is-manufacturing { color: var(--pdm-green); }
.org-node-kind { flex: 0 0 auto; min-width: 35px; padding: 2px 5px; border-radius: 4px; background: #eef3f9; color: var(--pdm-text-soft); font-size: 11px; text-align: center; }.org-node-kind.is-company { background: #e7f1ff; color: var(--pdm-blue); }.org-node-kind.is-BusinessDivision { background: #e6f7f2; color: var(--pdm-green); }.org-node-kind.is-Team { background: #fff3dc; color: var(--pdm-orange); }
...
.org-members-heading { display: flex; justify-content: space-between; margin: 4px 0 8px; }.org-members-heading h3 { margin: 0 0 3px; font-size: 12px; }.org-members-heading small { color: var(--pdm-muted); }
.org-table-scroll { width: 100%; min-height: 0; flex: 1 1 auto; overflow: auto; }.org-table-scroll table { width: 100%; min-width: 680px; }
...
.org-form { display: grid; gap: 14px; }.org-form label { display: grid; gap: 7px; color: var(--pdm-text-soft); font-size: 12px; }.org-form label.org-form-check { display: inline-flex; width: fit-content; min-height: 24px; align-items: center; justify-self: start; gap: 8px; cursor: pointer; }.org-form-check input[type='checkbox'] { width: 15px; height: 15px; flex: 0 0 auto; margin: 0; accent-color: var(--pdm-blue); }.org-form-check span { line-height: 15px; }.org-form input:not([type='checkbox']), .org-form select { min-height: 38px; border: 1px solid #d7dee9; border-radius: 6px; padding: 0 10px; background: #fff; }.org-form input:disabled, .org-form select:disabled { background: #f4f6f9; color: var(--pdm-muted); }.org-form-note { color: var(--pdm-muted); line-height: 1.4; }
.org-code-input { display: grid; grid-template-columns: auto minmax(0, 1fr); }.org-code-input > span { display: flex; align-items: center; border: 1px solid #d7dee9; border-right: 0; border-radius: 6px 0 0 6px; padding: 0 10px; background: #f4f6f9; color: var(--pdm-text-soft); font-family: var(--pdm-font-mono); }.org-form .org-code-input.has-prefix input { border-radius: 0 6px 6px 0; }.org-code-preview { color: var(--pdm-muted); font-weight: 400; }
.org-clear-manager { float: left; min-height: 34px; border: 1px solid #f2c5c0; border-radius: 5px; padding: 0 11px; background: #fff7f6; color: var(--pdm-danger); cursor: pointer; }
