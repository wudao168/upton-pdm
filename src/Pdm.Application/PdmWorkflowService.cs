    ...
    ReleaseDeliveryArchiveService? releaseDeliveryArchive = null)
{
    ...
    {
        _ = input;
        return PdmSystemSettings.DefaultReleaseChangeReasonTypes;
    }

    private static FormalSupplementPolicies NormalizeFormalSupplementPolicies(FormalSupplementPolicies? input)
    {
        input ??= FormalSupplementPolicies.Default;
        static FormalSupplementPolicy Normalize(FormalSupplementPolicy? policy, string label)
        {
            policy ??= FormalSupplementPolicy.Default;
            if (policy.MaximumCount is < 0 or > 99) throw new PdmRuleException($"{label}正式补充次数必须为0到99，或选择不限次数。");
            if (policy.ValidDays is < 1 or > 3650) throw new PdmRuleException($"{label}正式补充有效期必须为1到3650天，或选择不限时间。");
            return policy;
        }
        return new FormalSupplementPolicies(Normalize(input.Standard, "标准件BOM"), Normalize(input.Electrical, "电气BOM"));
    }

    private static string FormalSupplementPolicySummary(FormalSupplementPolicy policy) =>
        $"{(policy.MaximumCount.HasValue ? $"{policy.MaximumCount}次" : "不限次数")}，{(policy.ValidDays.HasValue ? $"{policy.ValidDays}天" : "不限时间")}";
    ...
    private static readonly ReleaseChangeReasonSelection[] ReleaseChangeReasonCatalog =
    [
        new("FormalSupplement", "FormalSupplement", "正式补充", "正式补充"),
        new("MaterialIssue", "LeadTimeUnsatisfied", "物料问题", "交期不满足"),
        new("MaterialIssue", "OrderedLate", "物料问题", "物料下单晚"),
        new("MaterialIssue", "WrongMaterial", "物料问题", "买错物料"),
        new("MaterialIssue", "MissedPurchase", "物料问题", "物料漏买"),
        new("DrawingIssue", "DrawingMissing", "图纸问题", "图纸漏下"),
        new("DrawingIssue", "DrawingError", "图纸问题", "图纸错误"),
        new("DesignIssue", "DesignChange", "设计问题", "设计变更"),
        new("DesignIssue", "DesignError", "设计问题", "设计错误"),
        new("CustomerReason", "RequirementChanged", "客户原因", "客户需求变更"),
        new("CustomerReason", "IncorrectInput", "客户原因", "客户信息输入错误"),
        new("CustomerReason", "ConfirmationDelayed", "客户原因", "客户未及时确认"),
        new("CustomerReason", "ProductNotProvided", "客户原因", "客户未及时提供产品")
    ];

    private sealed record NormalizedReleaseDescription(string Text, IReadOnlyList<ReleaseChangeReasonSelection> Selections);

    private static string ReleaseChangeReasonDisplay(ReleaseChangeReasonSelection selection) =>
        selection.CategoryCode == "FormalSupplement" ? "正式补充"
        : selection.CategoryCode == "Other" ? $"其他 / {selection.Detail}"
        : $"{selection.Category} / {selection.Reason}";

    private static NormalizedReleaseDescription NormalizeScopedReleaseDescription(string? input, ReleaseScope scope, IReadOnlyList<string> configuredReasonTypes)
    {
        _ = configuredReasonTypes;
        var value = input?.Trim() ?? string.Empty;
        if (IsSupplementScope(scope))
        {
            ...
            var normalized = new List<ReleaseChangeReasonSelection>();
            foreach (var reason in selected)
            {
                if (string.Equals(reason, "其他", StringComparison.OrdinalIgnoreCase))
                    throw new PdmRuleException("选择“其他”时必须填写具体原因。");
                if (reason.StartsWith("其他 / ", StringComparison.OrdinalIgnoreCase))
                {
                    var detail = reason[5..].Trim();
                    if (detail.Length == 0) throw new PdmRuleException("选择“其他”时必须填写具体原因。");
                    if (detail.Length > 200) throw new PdmRuleException("其他原因不能超过200个字符。");
                    if (detail.Contains('；')) throw new PdmRuleException("其他原因不能包含分号。");
                    normalized.Add(new ReleaseChangeReasonSelection("Other", "Other", "其他", "其他", detail));
                    continue;
                }
                var catalogItem = ReleaseChangeReasonCatalog.FirstOrDefault(item =>
                    string.Equals(ReleaseChangeReasonDisplay(item), reason, StringComparison.OrdinalIgnoreCase));
                if (catalogItem is null)
                {
                    if (ReleaseChangeReasonCatalog.Any(item => string.Equals(item.Category, reason, StringComparison.OrdinalIgnoreCase)))
                        throw new PdmRuleException($"请选择“{reason}”分类下的具体原因。");
                    throw new PdmRuleException($"变更原因“{reason}”不是有效的具体原因，请刷新后重试。");
                }
                normalized.Add(catalogItem);
            }
            normalized = normalized
                .DistinctBy(item => $"{item.CategoryCode}|{item.ReasonCode}|{item.Detail}", StringComparer.OrdinalIgnoreCase)
                .ToList();
            var text = string.Join('；', normalized.Select(ReleaseChangeReasonDisplay));
            if (text.Length > 1000) throw new PdmRuleException("变更原因合计不能超过1000个字符。");
            return new NormalizedReleaseDescription(text, normalized);
        }
        if (value.Length > 500) throw new PdmRuleException("备注不能超过500个字符。");
        return new NormalizedReleaseDescription(value, []);
    }
        ...
        Guid? drawingReviewWritebackId = null)
    {
        ...
        if (result.VersionCreated)
        {
            try
            {
                ...
                if (bomSnapshot is not null)
                    await EnsureBomChangeAllowedAsync(
                        document.ProjectId,
                        cancellationToken,
                        BomKind.Standard,
                        BomKind.NonStandard);
        ...
        CancellationToken cancellationToken)
    {
        ...
        var mechanical = standard.Concat(nonStandard).Concat(legacyMechanical).Where(IsPublishableBomItem).ToArray();
        ...
        var standardVersion = await ResolveBomVersionForReleaseAsync(projectId, BomKind.Standard, standard.Where(IsPublishableBomItem).ToArray(), actor, changeNumber, changeReason, effectiveSerialFrom, effectiveSerialTo, validationRules, cancellationToken);
        var nonStandardVersion = await ResolveBomVersionForReleaseAsync(projectId, BomKind.NonStandard, nonStandard.Where(IsPublishableBomItem).ToArray(), actor, changeNumber, changeReason, effectiveSerialFrom, effectiveSerialTo, validationRules, cancellationToken);
        var electricalVersion = await ResolveBomVersionForReleaseAsync(projectId, BomKind.Electrical, electrical.Where(IsPublishableBomItem).ToArray(), actor, changeNumber, changeReason, effectiveSerialFrom, effectiveSerialTo, validationRules, cancellationToken);
        ...
        int wholeSetMultiplier = 1)
    {
        ...
        changeNumber = IsSupplementScope(scope)
        ...
        if (snapshot is null && !IsLongLeadScope(scope))
            throw new PdmRuleException("项目尚无已存档的引用树快照，不能创建正式发布包；可先使用长交期BOM发布。");
        ...
        var settings = await repository.GetSystemSettingsAsync(cancellationToken);
        var normalizedDescription = NormalizeScopedReleaseDescription(changeReason, scope, settings.ReleaseChangeReasonTypes);
        changeReason = normalizedDescription.Text;
        FormalSupplementPolicy? formalSupplementPolicy = null;
        if (scope is ReleaseScope.StandardFormal or ReleaseScope.ElectricalFormal or ReleaseScope.NonStandardWithDrawing)
            formalSupplementPolicy = FormalSupplementPolicyForScope(settings.FormalSupplementPolicies, scope);
        else if (IsSupplementScope(scope))
            formalSupplementPolicy = await EnsureFormalSupplementPolicyAvailableAsync(
                projectId, scope, normalizedDescription.Selections, settings.FormalSupplementPolicies, cancellationToken);
        var standard = (await repository.GetBomAsync(projectId, BomKind.Standard, cancellationToken)).Where(IsPublishableBomItem).ToArray();
        var nonStandard = (await repository.GetBomAsync(projectId, BomKind.NonStandard, cancellationToken)).Where(IsPublishableBomItem).ToArray();
        var electrical = (await repository.GetBomAsync(projectId, BomKind.Electrical, cancellationToken)).Where(IsPublishableBomItem).ToArray();
        ...
        int wholeSetMultiplier = 1)
    {
        ...
        var settings = await repository.GetSystemSettingsAsync(cancellationToken);
        var normalizedDescription = NormalizeScopedReleaseDescription(changeReason, package.Scope, settings.ReleaseChangeReasonTypes);
        var normalizedReason = normalizedDescription.Text;
        wholeSetMultiplier = NormalizeWholeSetMultiplier(package.Scope, wholeSetMultiplier);
        FormalSupplementPolicy? formalSupplementPolicy = package.FormalSupplementPolicySnapshotted
            ? new FormalSupplementPolicy(package.FormalSupplementMaximumCount, package.FormalSupplementValidDays)
            : null;
        if (package.Scope is ReleaseScope.StandardFormal or ReleaseScope.ElectricalFormal or ReleaseScope.NonStandardWithDrawing)
            formalSupplementPolicy ??= FormalSupplementPolicyForScope(settings.FormalSupplementPolicies, package.Scope);
        else if (IsSupplementScope(package.Scope))
            formalSupplementPolicy = await EnsureFormalSupplementPolicyAvailableAsync(
                package.ProjectId, package.Scope, normalizedDescription.Selections, settings.FormalSupplementPolicies, cancellationToken, package.Id);
        ...
        if (IsLongLeadScope(package.Scope))
        {
            ...
            var selectedItems = SelectLongLeadItems(targetKind, currentItems, selectedBomItemIds, selectedBomItemQuantities);
            ...
            updated = package with
            {
                ...
                StandardBomRevision = targetKind == BomKind.Standard ? BomRevision("LL", selectedItems) : package.StandardBomRevision,
        ...
        CancellationToken cancellationToken)
    {
        ...
        await AuditAsync(actor, "release-package.long-lead-u9.retry", nameof(ReleasePackage), package.Id.ToString(),
            $"{package.Number}；BOM料号自动生成{generated.GeneratedCount}项、后台审批排队{generated.QueuedApprovalCount}项、已存在{generated.ExistingCount}项；{automation.Stage}；{automation.Message}", cancellationToken);
    ...
    public Task<BomItem> ApplyAutomaticallyMatchedMaterialCodeToBomAsync(Guid projectId, Guid itemId, string materialCode, string actor, CancellationToken cancellationToken) =>
        ApplyMaterialCodeToBomCoreAsync(projectId, itemId, materialCode, actor, true, cancellationToken);

    private async Task<BomItem> ApplyMaterialCodeToBomCoreAsync(
        Guid projectId,
        Guid itemId,
        string materialCode,
        string actor,
        bool allowMissingDrawingIdentity,
        CancellationToken cancellationToken)
    {
        ...
        if (current.EngineeringKitReferenceId.HasValue)
            throw new PdmRuleException($"套件“{current.EngineeringKitCode}”展开的子料不能单独更换料号；请重新引用套件。");
        ...
        await AuditAsync(actor, allowMissingDrawingIdentity ? "bom.material-code.auto-apply" : "bom.material-code.apply",
            nameof(BomItem), item.Id.ToString(), materialCode.Trim(), cancellationToken);
        return item;
    }
        ...
        CancellationToken cancellationToken)
    {
        ...
        var invalidKitLine = inputs.FirstOrDefault(input => input.EngineeringKitReferenceId.HasValue !=
            (input.EngineeringKitId.HasValue && input.EngineeringKitRevisionId.HasValue && input.EngineeringKitComponentId.HasValue
                && input.EngineeringKitVersionNumber.HasValue && !string.IsNullOrWhiteSpace(input.EngineeringKitCode)));
        if (invalidKitLine is not null) throw new PdmRuleException("套件展开行的追溯信息不完整，请重新引用套件。");
        var duplicateKitComponent = inputs.Where(input => input.EngineeringKitReferenceId.HasValue)
            .GroupBy(input => new { input.EngineeringKitReferenceId, input.EngineeringKitComponentId })
            .FirstOrDefault(group => group.Count() > 1);
        if (duplicateKitComponent is not null) throw new PdmRuleException("同一套件引用中不能重复包含相同子料。");
        var inconsistentKitReference = inputs.Where(input => input.EngineeringKitReferenceId.HasValue)
            .GroupBy(input => input.EngineeringKitReferenceId)
            .FirstOrDefault(group => group.Select(input => new
            {
                input.EngineeringKitId,
                input.EngineeringKitRevisionId,
                Code = input.EngineeringKitCode?.Trim().ToUpperInvariant(),
                input.EngineeringKitVersionNumber
            }).Distinct().Count() > 1);
        if (inconsistentKitReference is not null) throw new PdmRuleException("同一套件引用必须固定为同一个已发布版本。");

        var existing = await repository.GetBomAsync(projectId, kind, cancellationToken);
        var omittedRequiredKitItem = existing.FirstOrDefault(existingItem => existingItem.EngineeringKitReferenceId.HasValue
            && !existingItem.EngineeringKitComponentOptional && !existingItem.IsManuallyExcluded
            && inputs.Any(input => input.EngineeringKitReferenceId == existingItem.EngineeringKitReferenceId)
        ...
        var zeroQuantityDeletedAt = timeProvider.GetUtcNow();
        var zeroQuantityDeletedCount = inputs.Count(item => item.Quantity == 0);
        var items = inputs.OrderBy(item => item.Sequence).Select(input =>
        {
            ...
            var previous = input.Id.HasValue
                ...
                : input.SourceDocumentId.HasValue
                    ...
                    : null;
            if (previous?.EngineeringKitReferenceId is not null
                && (!string.Equals(previous.DrawingNumber, input.DrawingNumber?.Trim(), StringComparison.OrdinalIgnoreCase)
                    || !string.Equals(previous.Name, input.Name?.Trim(), StringComparison.Ordinal)
                    || previous.Quantity != input.Quantity
                    || !string.Equals(previous.Unit, U9UnitCatalog.NormalizeBomUnit(input.Unit), StringComparison.OrdinalIgnoreCase)
                    || !string.Equals(previous.Material ?? string.Empty, input.Material?.Trim() ?? string.Empty, StringComparison.Ordinal)
                    || !string.Equals(previous.Specification ?? string.Empty, input.Specification?.Trim() ?? string.Empty, StringComparison.Ordinal)
                    || !string.Equals(previous.Remark ?? string.Empty, input.Remark?.Trim() ?? string.Empty, StringComparison.Ordinal)
                    || !string.Equals(previous.Brand ?? string.Empty, input.Brand?.Trim() ?? string.Empty, StringComparison.Ordinal)
                    || !string.Equals(previous.SurfaceTreatment ?? string.Empty, input.SurfaceTreatment?.Trim() ?? string.Empty, StringComparison.Ordinal)
                    || !string.Equals(previous.Weight ?? string.Empty, input.Weight?.Trim() ?? string.Empty, StringComparison.Ordinal)))
                throw new PdmRuleException($"套件“{previous.EngineeringKitCode}”展开的子料编码、名称、数量和单位不能直接修改；请重新引用套件。");
            if (input.Sequence <= 0 || input.Quantity < 0)
                throw new PdmRuleException("BOM序号必须有效，数量不能小于0。");
            if (input.Quantity == 0)
            {
                if (previous is null) throw new PdmRuleException("新增BOM物料的数量必须大于0。");
                return previous with
                {
                    ...
                    ReconciliationStatus = previous.SourceDocumentId.HasValue ? ReconcileManuallyExcluded : ReconcileDeleted,
                    ReconciliationNote = $"已由{actor}将数量改为0并移入回收站。",
                    ReconciliationUpdatedBy = actor,
                    ReconciliationUpdatedAt = zeroQuantityDeletedAt,
                    DeletedAt = zeroQuantityDeletedAt,
                    DeletedBy = actor,
                    DeleteReason = "数量修改为0，系统自动删除"
                };
            }
            if ((kind == BomKind.Electrical && string.IsNullOrWhiteSpace(input.DrawingNumber))
                ...
                false)
            {
                ...
                EngineeringKitReferenceId = previous?.EngineeringKitReferenceId ?? input.EngineeringKitReferenceId,
                EngineeringKitId = previous?.EngineeringKitId ?? input.EngineeringKitId,
                EngineeringKitRevisionId = previous?.EngineeringKitRevisionId ?? input.EngineeringKitRevisionId,
                EngineeringKitCode = previous?.EngineeringKitCode ?? NullIfWhiteSpace(input.EngineeringKitCode),
                EngineeringKitVersionNumber = previous?.EngineeringKitVersionNumber ?? input.EngineeringKitVersionNumber,
                EngineeringKitComponentId = previous?.EngineeringKitComponentId ?? input.EngineeringKitComponentId,
                EngineeringKitComponentOptional = previous?.EngineeringKitReferenceId.HasValue == true
                    ? previous.EngineeringKitComponentOptional
                    : input.EngineeringKitComponentOptional,
                Source = previous?.Source ?? (input.EngineeringKitReferenceId.HasValue ? "EngineeringKit" : "Manual"),
                ...
                ReconciliationNote = previous?.ReconciliationNote ?? (previous is null
                    ? input.EngineeringKitReferenceId.HasValue
                        ? $"由套件 {input.EngineeringKitCode} / V{input.EngineeringKitVersionNumber:D2} 展开。"
                        : "人工新增物料，不来源于当前图档源数据。"
                    : null),
        ...
        return generated.StandardItems
            ...
            .Select((item, index) =>
            {
                ...
                if (current is null)
                {
                    ...
                    var classified = item.Kind is BomKind.Standard or BomKind.NonStandard;
                    return item with
                    {
                        Sequence = index + 1,
                        IsComplete = classified && HasRequiredBomValues(item, item.Kind, validationRules),
                        ReconciliationStatus = classified ? "PendingBomSync" : ReconcilePendingClassification,
                        ReconciliationNote = classified
                            ? $"已识别为{BomKindLabel(item.Kind)}，尚未同步到维护BOM。"
    ...
    public async Task<IReadOnlyList<BomItem>> SetBomReleaseExclusionAsync(
        Guid projectId,
        SetBomReleaseExclusionCommand command,
        string actor,
        UserRole role,
        CancellationToken cancellationToken)
    {
        ...
        if (itemIds.Count > 500) throw new PdmRuleException("单次最多处理500条BOM物料。");
        var reason = command.Reason?.Trim();
        if (command.Excluded && string.IsNullOrWhiteSpace(reason)) throw new PdmRuleException("请填写不发布原因。");
        if (reason?.Length > 500) throw new PdmRuleException("不发布原因不能超过500个字符。");
        ...
        if (!command.Excluded && selected.Any(item => !item.IsReleaseExcluded))
            throw new PdmRuleException("选中的物料中包含正在参与发布的物料，请分开处理。");
        await EnsureBomChangeAllowedAsync(projectId, cancellationToken, selected.Select(item => item.Kind).ToArray());

        IReadOnlyList<BomItem> Apply(IEnumerable<BomItem> source) => source.Select(item => !itemIds.Contains(item.Id)
            ? item
            : item with
            {
                IsReleaseExcluded = command.Excluded,
                ReleaseExclusionReason = command.Excluded ? reason : null
            }).ToArray();
        ...
        var action = command.Excluded ? "bom.release-exclusion.set" : "bom.release-exclusion.restore";
        var label = command.Excluded ? "不发布" : "恢复发布";
        var detail = $"{label}{selected.Length}条；物料：{string.Join('、', selected.Select(item => item.DrawingNumber))}"
            + (command.Excluded ? $"；原因：{reason}" : string.Empty);
        var audit = new AuditEntry(Guid.NewGuid(), changedAt, actor, action, nameof(BomItem), projectId.ToString(), detail);
        await repository.ApplyBomBatchAsync(projectId, updatedStandard, updatedNonStandard, unclassified, updatedElectrical, virtualItems, [], [audit], cancellationToken);
        foreach (var changedKind in selected.Select(item => item.Kind).Distinct())
            await SyncBomDraftAsync(projectId, changedKind, actor, cancellationToken);
        return updatedStandard.Concat(updatedNonStandard).Concat(updatedElectrical).ToArray();
    }
    ...
    public async Task<IReadOnlyList<ApprovalTransferCandidate>> ListDrawingReviewersAsync(Guid projectId, string actor, UserRole role, CancellationToken cancellationToken)
    {
        ...
        var eligible = await ListDrawingReviewCandidateUsersAsync(projectId, actor, cancellationToken);
        return eligible
            .Select(user => new ApprovalTransferCandidate(user.Username, user.DisplayName))
    ...
    public async Task<DrawingReviewPackage> CreateDrawingReviewPackageAsync(Guid projectId, IReadOnlyCollection<Guid>? modelDocumentIds, IReadOnlyCollection<string>? assignedReviewers, string actor, UserRole role, CancellationToken cancellationToken)
    {
        await RequirePermissionAsync(actor, role, PermissionCodes.DrawingReviewSubmit, cancellationToken);
        if (!await repository.HasProjectContentReadAccessAsync(projectId, actor, role, cancellationToken))
            throw new UnauthorizedAccessException("当前用户没有该项目的操作权限。");
        ...
        else
        {
            ...
            if (requested.Length == 0) throw new PdmRuleException("请至少选择一张2D工程图进行审核。");
        ...
        var reviewers = new List<UserAccount>();
        foreach (var username in (assignedReviewers ?? [])
                     .Where(item => !string.IsNullOrWhiteSpace(item))
                     .Select(item => item.Trim())
                     .Distinct(StringComparer.OrdinalIgnoreCase))
        {
            var reviewer = await repository.FindUserAsync(username, cancellationToken)
                ?? throw new PdmRuleException($"指定审核人 {username} 不存在。");
        ...
        var supervisor = await repository.FindUserAsync(supervisorUsername, cancellationToken)
            ?? throw new PdmRuleException("已配置的机械主管账号不存在。");
        if (!supervisor.IsActive) throw new PdmRuleException("已配置的机械主管账号已停用。");
        if (reviewers.Any(reviewer => string.Equals(supervisor.Username, reviewer.Username, StringComparison.OrdinalIgnoreCase)))
        ...
        var items = selected.Select(candidate => new DrawingReviewItem
        {
            ...
            ModelState = DrawingReviewTargetState.NotRequired,
        ...
        var package = new DrawingReviewPackage
        {
            ...
            CreatedBy = actor,
            CreatedAt = now,
            AssignedReviewers = reviewers.Select(reviewer => reviewer.Username).ToArray(),
            AssignedReviewerNames = reviewers.Select(reviewer => reviewer.DisplayName).ToArray(),
        ...
        await AuditAsync(actor, "drawing-review.create", nameof(DrawingReviewPackage), package.Id.ToString(),
            $"{package.Number}；选择2D工程图 {package.Items.Count}张；{(reviewers.Count == 0 ? "未指定审核人，由具备审图权限的人员并行处理" : $"指定审核人 {string.Join("、", reviewers.Select(reviewer => reviewer.DisplayName))}")}；机械主管 {supervisor.DisplayName}", cancellationToken);
        var reviewerRecipients = reviewers.Count > 0
            ? reviewers.Select(reviewer => reviewer.Username)
            : (await ListDrawingReviewCandidateUsersAsync(projectId, actor, cancellationToken)).Select(user => user.Username);
        await CreateDrawingReviewNotificationsAsync(projectId, $"drawing-review:{package.Id:N}:reviewer", "DrawingReviewAssigned", "待审图纸",
            project => $"{project.Code} · {package.Number} 有 {package.Items.Count} 张2D工程图待审核，发起人 {actor}",
            reviewerRecipients, cancellationToken);
        return package;
    }
    ...
    private async Task<IReadOnlyList<DrawingReviewCandidateBuild>> BuildDrawingReviewCandidatesAsync(Guid projectId, CancellationToken cancellationToken)
    {
        ...
        var sources = effectiveBomItems
            ...
            .Select(group =>
            {
                var preferred = group.OrderBy(item => item.Sequence).First();
                return new DrawingReviewCandidateSource(preferred.Id, preferred.DrawingNumber, preferred.Name, preferred.SourceConfiguration,
                    preferred.SourceDocumentId!.Value, [BomKind.NonStandard]);
            })
            .ToList();
        ...
        foreach (var source in sources.OrderBy(source => source.DrawingNumber, StringComparer.OrdinalIgnoreCase))
        {
            ...
            var relatedDrawings = relatedDrawingIds
                ...
                .ToArray();
            var drawing = relatedDrawings.Length == 1 ? relatedDrawings[0] : null;
            ...
            else if (drawing is not null && activeDocumentIds.Contains(drawing.Id))
            {
                state = DrawingReviewCandidateState.InReview;
                reason = "已在其他审核单中";
            }
            else if (drawing is not null && !string.IsNullOrWhiteSpace(drawing.CheckedOutBy))
            {
                state = DrawingReviewCandidateState.Unavailable;
                reason = "图档仍处于签出编辑状态";
            }
    ...
    public async Task<DrawingReviewPackage> AddDrawingReviewMarkupAsync(Guid packageId, AddDrawingReviewMarkupCommand command, string actor, UserRole role, CancellationToken cancellationToken)
    {
        ...
        var item = package.Items.SingleOrDefault(item => item.Id == command.ItemId);
        if (item is null)
            throw new PdmNotFoundException("图纸审核项不存在。");
        if (command.Target != DrawingReviewTarget.Drawing2D)
            throw new PdmRuleException("当前流程仅允许在2D工程图上添加批注。");
        if (!item.RequiresDrawingReview)
    ...
    public async Task<DrawingReviewPackage> DecideDrawingReviewTargetAsync(Guid packageId, Guid itemId, DecideDrawingReviewTargetCommand command, string actor, UserRole role, CancellationToken cancellationToken)
    {
        ...
        var item = package.Items.SingleOrDefault(candidate => candidate.Id == itemId)
            ?? throw new PdmNotFoundException("图纸审核项不存在。");
        if (command.Target != DrawingReviewTarget.Drawing2D)
            throw new PdmRuleException("当前流程仅审核2D工程图。");
        if (!item.RequiresDrawingReview)
            throw new PdmRuleException("该非标件没有唯一关联的2D工程图。");
        ...
        else if (!package.AllowsReviewer(actor) && !developerSelfReviewAllowed)
        {
            var assigned = package.AssignedReviewerNames.Count > 0
                ...
                : string.Join("、", package.AssignedReviewers);
        ...
        if (string.IsNullOrWhiteSpace(package.Supervisor))
        {
            // 升级前发起且未指定审核人的审核单没有机械主管节点，沿用原有单级流程直接进入属性写回。
            var legacyWritebacks = BuildDrawingReviewWritebacks(package, actor, reviewerName, now);
            package = await repository.QueueDrawingReviewWritebacksAsync(package.Id, legacyWritebacks, cancellationToken);
            await AuditAsync(actor, "drawing-review.writeback.queue", nameof(DrawingReviewPackage), package.Id.ToString(), $"2D图纸审核属性写回{legacyWritebacks.Length}项", cancellationToken);
            return package;
        }
        ...
        await AuditAsync(actor, "drawing-review.supervisor.submit", nameof(DrawingReviewPackage), package.Id.ToString(),
            $"审图节点完成；转机械主管 {package.SupervisorName ?? package.Supervisor}", cancellationToken);
        await CreateDrawingReviewNotificationsAsync(package.ProjectId, $"drawing-review:{package.Id:N}:supervisor", "DrawingReviewSupervisorApproval", "待机械主管批准",
            project => $"{project.Code} · {package.Number} 已完成审图，等待机械主管批准",
            [package.Supervisor], cancellationToken);
        return package;
    }

    private async Task<DrawingReviewPackage> RevokeDrawingReviewTargetAsync(
        DrawingReviewPackage package,
        DrawingReviewItem item,
        string actor,
        DecideDrawingReviewTargetCommand command,
        CancellationToken cancellationToken)
    {
        if (package.State is not (DrawingReviewPackageState.InReview or DrawingReviewPackageState.PendingSupervisorApproval))
            throw new PdmConflictException("当前图纸审核单不允许撤销审核结论。");
        if (item.DrawingState is not (DrawingReviewTargetState.Approved or DrawingReviewTargetState.Marked or DrawingReviewTargetState.ChangesRequested))
            throw new PdmRuleException("该2D工程图尚未给出审核结论，无需撤销。");
        var developerSelfReviewAllowed = TenantContext.Current?.HasRole("developer") == true;
        if (!package.AllowsReviewer(actor)
            && !string.Equals(package.Supervisor, actor, StringComparison.OrdinalIgnoreCase)
            && !developerSelfReviewAllowed)
            throw new UnauthorizedAccessException("只有本审核单的审图人或机械主管可以撤销审核结论。");
        var comment = string.IsNullOrWhiteSpace(command.Comment) ? null : command.Comment.Trim();
    ...
    private static DrawingReviewWritebackRequest[] BuildDrawingReviewWritebacks(DrawingReviewPackage package, string actor, string reviewerName, DateTimeOffset now) =>
        package.Items.Select(candidate =>
        {
            ...
            var drawingProperties = DrawingReviewProperties(package, DrawingReviewTarget.Drawing2D,
                candidate.DrawingReviewer ?? actor, candidate.DrawingReviewerName ?? reviewerName,
                candidate.DrawingReviewedAt ?? now, candidate.DrawingRevision!);
    ...
    public async Task<CadPropertyWriteback> CompleteCadPropertyWritebackAsync(Guid id, Guid resultVersionId, string actor, UserRole role, CancellationToken cancellationToken)
    {
        ...
        if (review?.State == DrawingReviewPackageState.Approved)
            await AuditAsync(actor, "drawing-review.approved", nameof(DrawingReviewPackage), review.Id.ToString(), $"{review.Number}；2D图纸审核属性已写回", cancellationToken);
        return updated;
    }
    ...
    public async Task<ReleasePackage> SubmitReleasePackageAsync(Guid releasePackageId, string actor, UserRole role, CancellationToken cancellationToken)
    {
        ...
        var normalizedDescription = NormalizeScopedReleaseDescription(
            package.ChangeReason ?? "兼容既有发布流程创建的设变", package.Scope, settings.ReleaseChangeReasonTypes);
        var changeReason = normalizedDescription.Text;
        FormalSupplementPolicy? formalSupplementPolicy = package.FormalSupplementPolicySnapshotted
            ? new FormalSupplementPolicy(package.FormalSupplementMaximumCount, package.FormalSupplementValidDays)
            : null;
        if (package.Scope is ReleaseScope.StandardFormal or ReleaseScope.ElectricalFormal or ReleaseScope.NonStandardWithDrawing)
        ...
        if (package.State == ReleasePackageState.Draft
            && (package.ChangeReason != normalizedPackage.ChangeReason
                || !package.ChangeReasonSelections.SequenceEqual(normalizedPackage.ChangeReasonSelections)
                || package.FormalSupplementPolicySnapshotted != normalizedPackage.FormalSupplementPolicySnapshotted
                || package.FormalSupplementMaximumCount != normalizedPackage.FormalSupplementMaximumCount
                || package.FormalSupplementValidDays != normalizedPackage.FormalSupplementValidDays))
            package = await repository.UpdateDraftReleasePackageAsync(normalizedPackage, cancellationToken);
        else
        ...
        {
            var standard = (await repository.GetBomAsync(package.ProjectId, BomKind.Standard, cancellationToken)).Where(IsPublishableBomItem).ToArray();
            var nonStandard = (await repository.GetBomAsync(package.ProjectId, BomKind.NonStandard, cancellationToken)).Where(IsPublishableBomItem).ToArray();
            var electrical = (await repository.GetBomAsync(package.ProjectId, BomKind.Electrical, cancellationToken)).Where(IsPublishableBomItem).ToArray();
    ...
    public async Task<IReadOnlyList<Guid>> GetNonStandardItemsForFinalApprovalAsync(Guid taskId, CancellationToken cancellationToken)
    {
        var package = await repository.FindReleasePackageByApprovalTaskAsync(taskId, cancellationToken)
            ?? throw new PdmNotFoundException("审批任务不存在。");
        if (package.Scope != ReleaseScope.LegacyCombined && !IsNonStandardScope(package.Scope)) return [];
        var task = package.ApprovalTasks.FirstOrDefault(item => item.Id == taskId)
            ?? throw new PdmNotFoundException("审批任务不存在。");
        ...
        bool preserveManualOverrides = true)
    {
        ...
        foreach (var item in generated)
        {
            ...
            merged.Add(preserveManualOverrides && previous?.IsManuallyOverridden == true
                ...
                : item with
                {
                    ...
                    IsReleaseExcluded = preserveManualOverrides && previous?.IsReleaseExcluded == true,
                    ReleaseExclusionReason = preserveManualOverrides ? previous?.ReleaseExclusionReason : null,
        ...
        if (apply)
        {
            ...
            await repository.SaveBomDraftAsync(projectId, BomKind.Standard, standard.Where(IsPublishableBomItem).ToArray(), actor, cancellationToken);
            await repository.SaveBomDraftAsync(projectId, BomKind.NonStandard, nonStandard.Where(IsPublishableBomItem).ToArray(), actor, cancellationToken);
    ...
    private async Task EnsureStandardMaterialIdentityAsync(BomItem item, string materialCode, bool allowMissingDrawingIdentity, CancellationToken cancellationToken)
    {
        ...
        var issues = allowMissingDrawingIdentity
            ? MaterialService.StandardBomMaterialMasterBlockingIssues(item, material)
            : MaterialService.StandardBomMaterialMasterIssues(item, material);
    ...
    /// <summary>
    /// 图纸审核节点的站内消息：审图人、机械主管与发起人按节点变化各收到一条，同一节点重复触发不会重复提醒。
    /// </summary>
    private async Task CreateDrawingReviewNotificationsAsync(
        Guid projectId,
        string sourceKey,
        string category,
        string title,
        Func<Project, string> contentFactory,
        ...
        CancellationToken cancellationToken)
    {
        ...
        var content = contentFactory(project);
        await repository.CreateUserNotificationsAsync(usernames.Select(username => new UserNotification(
            Guid.NewGuid(),
            username,
            category,
            title,
    ...
    {
        var included = items.Where(IsPublishableBomItem).ToArray();
        ...
        IReadOnlyList<BomItem>? currentItems = null)
    {
        ...
        if (withoutDrawingSource.Length > 0)
            throw new PdmRuleException($"以下非标件没有来源模型，无法定位2D工程图，不能进入非标件BOM审核发布：{string.Join("、", withoutDrawingSource.Take(12))}。");
        ...
        foreach (var item in effectiveItems)
        {
            ...
            if (requireExactQuantities && item.Quantity != source.Quantity)
                ...
                foreach (var package in approvedPackages)
                {
                                 ...
                                 && MatchesReviewedVersion(latestDrawing, item.EffectiveDrawingVersionId.Value, drawingVersions)))
                    {
                        if (package.Markups.Any(markup => markup.ItemId == reviewItem.Id
                                && markup.Target == DrawingReviewTarget.Drawing2D
                                && markup.Severity == DrawingReviewMarkupSeverity.Blocking
                                && markup.State == DrawingReviewMarkupState.Open))
                            continue;
                        matched = true;
    ...
    {
        if (IsLongLeadScope(scope)) return 1;
        if (multiplier is < 1 or > 1000)
            throw new PdmRuleException("整套倍率必须是1到1000之间的整数。");
        return multiplier;
    }
    ...
    private async Task EnsureReleaseScopeAvailableAsync(
        ...
        IReadOnlyCollection<BomItem> currentBomItems,
        CancellationToken cancellationToken,
        Guid? excludedReleasePackageId = null)
    {
        ...
        {
            if (packages.Any(package => package.Scope == formalScope && package.State == ReleasePackageState.Published))
                throw new PdmRuleException($"{BomKindLabel(targetKind)}正式版已发布，不能再提前发布长交期件，请使用增补/变更。");
            if (packages.Any(package => package.Scope == formalScope
        ...
        var active = packages.FirstOrDefault(package =>
            ...
            && (package.Scope == ReleaseScope.LegacyCombined && PackageBomVersionIds(package).Length == 3
                || package.Scope != ReleaseScope.LegacyCombined && !IsLongLeadScope(package.Scope) && ReleaseScopeBomKind(package.Scope) == targetKind));
        if (active is not null)
            throw new PdmConflictException($"发布包{active.Number}正在审批或发布，{BomKindLabel(targetKind)}BOM暂时锁定。");
    }

    private static FormalSupplementPolicy FormalSupplementPolicyForScope(FormalSupplementPolicies policies, ReleaseScope scope) =>
        scope is ReleaseScope.StandardFormal or ReleaseScope.StandardSupplement or ReleaseScope.NonStandardWithDrawing or ReleaseScope.NonStandardSupplement ? policies.Standard
        : scope is ReleaseScope.ElectricalFormal or ReleaseScope.ElectricalSupplement ? policies.Electrical
        : throw new ArgumentOutOfRangeException(nameof(scope), scope, "发布范围不适用正式补充规则。");

    private static bool ContainsFormalSupplementReason(ReleasePackage package) =>
        package.ChangeReasonSelections.Any(item => item.CategoryCode == "FormalSupplement")
        || (package.ChangeReason ?? string.Empty).Split('；', StringSplitOptions.RemoveEmptyEntries | StringSplitOptions.TrimEntries)
            .Any(item => string.Equals(item, "正式补充", StringComparison.OrdinalIgnoreCase));

    private async Task<FormalSupplementPolicy> EnsureFormalSupplementPolicyAvailableAsync(
        Guid projectId,
        ReleaseScope supplementScope,
        IReadOnlyList<ReleaseChangeReasonSelection> selections,
        FormalSupplementPolicies configuredPolicies,
        CancellationToken cancellationToken,
        Guid? excludedReleasePackageId = null)
    {
        var formalScope = supplementScope == ReleaseScope.StandardSupplement
            ? ReleaseScope.StandardFormal
            : supplementScope == ReleaseScope.ElectricalSupplement
                ? ReleaseScope.ElectricalFormal
                : supplementScope == ReleaseScope.NonStandardSupplement
                    ? ReleaseScope.NonStandardWithDrawing
                : throw new ArgumentOutOfRangeException(nameof(supplementScope), supplementScope, "发布范围不是增补/变更。");
        var packages = (await repository.ListReleasePackagesAsync(projectId, cancellationToken))
            .Where(package => package.Id != excludedReleasePackageId).ToArray();
        var formal = packages
            .Where(package => package.Scope == formalScope && package.State == ReleasePackageState.Published && package.PublishedAt.HasValue)
            .OrderBy(package => package.PublishedAt)
            .FirstOrDefault()
            ?? throw new PdmRuleException($"{BomKindLabel(ReleaseScopeBomKind(supplementScope))}BOM尚未完成首次正式发布，不能发起增补/变更。");
        var policy = formal.FormalSupplementPolicySnapshotted
            ? new FormalSupplementPolicy(formal.FormalSupplementMaximumCount, formal.FormalSupplementValidDays)
            : FormalSupplementPolicyForScope(configuredPolicies, supplementScope);
        if (!selections.Any(item => item.CategoryCode == "FormalSupplement")) return policy;

        if (policy.ValidDays is int validDays)
        {
            var validUntil = formal.PublishedAt!.Value.AddDays(validDays);
            if (timeProvider.GetUtcNow() > validUntil)
                throw new PdmRuleException($"正式补充有效期已于{validUntil.ToOffset(TimeSpan.FromHours(8)):yyyy-MM-dd HH:mm}结束，请选择实际问题原因。");
        }
        if (policy.MaximumCount is int maximumCount)
        {
            var occupied = packages.Count(package =>
                package.Scope == supplementScope
                && package.State is ReleasePackageState.Published or ReleasePackageState.ProcessReview or ReleasePackageState.Approval or ReleasePackageState.Publishing
                && ContainsFormalSupplementReason(package));
            if (occupied >= maximumCount)
                throw new PdmRuleException($"正式补充次数已用完（{occupied}/{maximumCount}），请选择实际问题原因。");
        }
        return policy;
    }
    ...
    private async Task EnsureBomChangeAllowedAsync(Guid projectId, CancellationToken cancellationToken, params BomKind[] changedKinds)
    {
        ...
        var active = (await repository.ListReleasePackagesAsync(projectId, cancellationToken)).FirstOrDefault(package =>
            ...
            && (package.Scope == ReleaseScope.LegacyCombined && PackageBomVersionIds(package).Length == 3
                || package.Scope != ReleaseScope.LegacyCombined && !IsLongLeadScope(package.Scope)
    ...
    {
        var items = (await repository.GetBomAsync(projectId, kind, cancellationToken)).Where(IsPublishableBomItem).ToArray();
    ...
    private static BomItem[] SelectLongLeadItems(
        BomKind kind,
        IReadOnlyList<BomItem> currentItems,
        IReadOnlyList<Guid>? selectedBomItemIds,
        IReadOnlyDictionary<Guid, decimal>? selectedBomItemQuantities)
    {
        ...
        if (selectedIds.Count == 0) throw new PdmRuleException($"长交期发布必须至少选择一个{label}物料。");
        var selectedItems = currentItems.Where(item => selectedIds.Contains(item.Id)).ToArray();
        if (selectedItems.Length != selectedIds.Count)
            throw new PdmRuleException($"长交期发布选择中包含已删除或不属于{label}BOM的物料。");
        if (selectedBomItemQuantities is null) return selectedItems;
        if (selectedBomItemQuantities.Keys.Any(id => !selectedIds.Contains(id)))
            throw new PdmRuleException($"长交期发布数量中包含未选择的{label}物料。");
        ...
        IReadOnlyList<Guid>? selectedBomItemIds)
    {
        ...
        if (partiallySelectedGroup is not null)
            throw new PdmRuleException($"同一料号和单位的物料必须整组选择，物料{partiallySelectedGroup.First().DrawingNumber}存在未同时选择的BOM实例。");
        return selectedItems;
    }

    private static bool BomSnapshotsEqual(IReadOnlyList<BomItem> left, IReadOnlyList<BomItem> right) =>
        left.Select(ForReleaseComparison).OrderBy(item => item.Id)
            .SequenceEqual(right.Select(ForReleaseComparison).OrderBy(item => item.Id));
